...
def getCategoryItems(category_name):
    ...
    categories = session.query(Category).all()
    selected_category = session.query(Category).filter_by(name=category_name).one()  # noqa
    items = session.query(CategoryItem).filter_by(category_id=selected_category.id).all()  # noqa
    category_names = {}
    for category in categories:
        category_names[category.id] = category.name
    if len(items) == 0:
    ...
    try:
        ...
        user = None
    return render_template(
        'category_detail.html', selected_category=selected_category,
        user=user,
...
def deleteItem(item_title):
    ...
    if request.method == 'POST':
        ...
        user = login_session['username']
        return render_template(
                               'delete_item.html', item_title=item_title,
                               user=user
        )
